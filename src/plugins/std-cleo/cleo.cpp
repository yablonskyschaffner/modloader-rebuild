...
class CThePlugin : public modloader::CPlugin
{
    ...
    public:
        ...
        struct CacheStruct
        {
            ...
            {
                SetupFlags(file);
                snprintf(srcPath, sizeof(srcPath), "%s", GetFilePath(file).c_str());
                snprintf(dstPath, sizeof(dstPath), "%s%s",
                         !flags.bIsFXT? "CLEO\\" : "CLEO\\CLEO_TEXT\\", file.filename);
                PosConstruct();
            }
            ...
            {
                SetupFlags(file);
                snprintf(srcPath, sizeof(srcPath), "%s%s", GetFilePath(modir).c_str(), file.filepath);
                snprintf(dstPath, sizeof(dstPath), "CLEO\\%s", file.filepath);
                PosConstruct();
            }
            ...
            void PosConstruct()
            {
                ...
                if(flags.bIsCLEO)
                {
                    ...
                    {
                        const size_t remaining = sizeof(dstPath) - (p + 1 - dstPath);
                        snprintf(p+1, remaining, "_cleo_");
                    }
                }
            }
...
bool CThePlugin::OnStartup()
{
    ...
    /* Setup vars */
    snprintf(cacheDirPath, sizeof(cacheDirPath), "%s%s", this->modloader->cachepath, "std-cleo\\");
    snprintf(cacheFilePath, sizeof(cacheFilePath), "%s%s", cacheDirPath, "cache.bin");
    snprintf(cacheCleoPath, sizeof(cacheCleoPath), "%s%s", cacheDirPath, "CLEO\\");
    ...
    else
    {
        ...
        while(fread(&file, sizeof(file), 1, f) == 1)
        {
            ...
            if(file.flags.bExists)
            {
                ...
                char src[MAX_PATH];
                snprintf(src, sizeof(src), "%s%s", cacheDirPath, file.dstPath);
