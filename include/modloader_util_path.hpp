...
namespace modloader
{
    ...
    {
        if(CreateDirectoryA(szTo, nullptr))
        {
            ForeachFile(szFrom, "*.*", false, [&szFrom, &szTo](ModLoaderFile& file)
            {
                ...
                snprintf(szToFile, sizeof(szToFile), "%s\\%s", szTo, pPath);
                snprintf(szFromFile, sizeof(szFromFile), "%s\\%s", szFrom, pPath);
    ...
    inline BOOL DestroyDirectoryA(LPCTSTR szPath)
    {
        ForeachFile(szPath, "*.*", false, [&szPath](ModLoaderFile& file)
        {
            ...
            snprintf(szPathFile, sizeof(szPathFile), "%s\\%s", szPath, pPath);
