...
namespace modloader
{
    ...
    {
        std::transform(str.begin(), str.end(), str.begin(),
                       [](unsigned char c) -> char { return static_cast<char>(::toupper(c)); });
        return str;
    }
    ...
    {
        for(char* p = str; *p; ++p) *p = static_cast<char>(::tolower(static_cast<unsigned char>(*p)));
        return str;
    }
    ...
    {
        for(char* p = str; *p; ++p) *p = static_cast<char>(::toupper(static_cast<unsigned char>(*p)));
        return str;
    }
